...
async function findBusinessWithSerpAPI(businessName) {
  try {
    ...
    console.log('Search response keys:', Object.keys(searchResponse.data));
    ...
    if (!places || places.length === 0) {
      ...
      console.log('Full response:', JSON.stringify(searchResponse.data, null, 2).substring(0, 500));
      return null;
    }
  ...
  try {
    ...
    if (places && places.length > 0) {
      ...
      res.json({ 
        success: true, 
        found: false,
        responseKeys: Object.keys(searchResponse.data),
        hasPlaceResults: !!searchResponse.data.place_results
      });
    }
  } catch (err) {
    res.status(500).json({ 
