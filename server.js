...
function parseRelativeDate(relativeStr) {
  ...
  // Handle "a X ago" and "X Ys ago" patterns
  if (str.includes('hour') || str.includes('minute') || str.includes('second')) {
    return now; // Very recent, today
  }
...
async function findBusinessWithSerpAPI(businessName) {
  try {
    ...
    // Step 2: Get detailed info including reviews using data_id
    if (place.data_id) {
      try {
        ...
        console.log('Could not get reviews, using place data only');
        return {
          name: place.title,
          address: place.address || 'Service area business',
          rating: place.rating || 0,
    ...
    // Return basic info if no data_id
    return {
      name: place.title,
      address: place.address || 'Service area business',
      rating: place.rating || 0,
...
app.post('/api/calculate-rot', async (req, res) => {
  try {
    ...
    if (place.reviews && place.reviews.length > 0) {
      // SerpAPI returns reviews sorted by newest first
      // Reviews have a "date" field like "a month ago", "2 weeks ago"
  ...
  try {
    ...
    if (place) {
      res.json({
        ...
        reviewsReturned: place.reviews ? place.reviews.length : 0,
        newestReviewDate: place.reviews && place.reviews[0] ? place.reviews[0].date : null
      });
    } else {
